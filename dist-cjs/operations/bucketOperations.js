...
exports.verifyBucketCreation = verifyBucketCreation;
exports.waitForBackendBucketReady = waitForBackendBucketReady;
...
async function createBucket(bucketName) {
    ...
    const { mspId } = await (0, mspService_js_1.getMspInfo)();
    // Choose one of the value props retrieved from the MSP through the helper function
    const valuePropId = await (0, mspService_js_1.getValueProps)();
    console.log(`Value Prop ID: ${valuePropId}`);
    // Derive bucket ID
    const bucketId = (await clientService_js_1.storageHubClient.deriveBucketId(clientService_js_1.address, bucketName));
    console.log(`Derived bucket ID: ${bucketId}`);
    // Check that the bucket doesn't exist yet
    const bucketBeforeCreation = await clientService_js_1.polkadotApi.query.providers.buckets(bucketId);
    ...
    const txHash = await clientService_js_1.storageHubClient.createBucket(mspId, bucketName, isPrivate, valuePropId);
    console.log('createBucket() txHash:', txHash);
    if (!txHash) {
        throw new Error('createBucket() did not return a transaction hash');
    }
    // Wait for transaction receipt
    const txReceipt = await clientService_js_1.publicClient.waitForTransactionReceipt({
        hash: txHash,
    });
...
async function verifyBucketCreation(bucketId) {
    const { mspId } = await (0, mspService_js_1.getMspInfo)();
    const bucket = await clientService_js_1.polkadotApi.query.providers.buckets(bucketId);
    if (bucket.isEmpty) {
        throw new Error('Bucket not found on chain after creation');
    }
    const bucketData = bucket.unwrap().toHuman();
    console.log('Bucket userId matches initial bucket owner address', bucketData.userId === clientService_js_1.address);
    console.log(`Bucket MSPId matches initial MSPId: ${bucketData.mspId === mspId}`);
    return bucketData;
}
async function waitForBackendBucketReady(bucketId) {
    ...
    for (let i = 0; i < maxAttempts; i++) {
        ...
        try {
            ...
            const bucket = await mspService_js_1.mspClient.buckets.getBucket(bucketId);
            if (bucket) {
